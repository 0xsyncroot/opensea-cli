#!/usr/bin/env node
import {
  JsonRpcProvider, Wallet, formatEther, parseUnits,
} from 'ethers';
...
import {
  clockDrift, waitUntilUnix, waitUntilBlock,
...
async function adaptivePriority(provider: JsonRpcProvider, cfg: Config): Promise<bigint | undefined> {
  if (!cfg.adaptive) return undefined;
  ...
  if (bid > cap) log.warn(`bid capped at ${cfg.maxPriorityFeeGwei} gwei`);
  return capped;
}

async function watchInclusion(
  provider: JsonRpcProvider, txHash: string, startBlock: number, windowBlocks: number,
) {
...
function printMintBanner(success: boolean) {
  const bar = '═'.repeat(48);
  console.log();
  if (success) {
...
async function checkMode(cfg: Config) {
  ...
  } else if (total) log.kv('supply', `${total} minted`);
  if (price && price !== '0') {
    try { log.kv('mint price', chalk.bold(formatEther(BigInt(price)) + ' ETH') + chalk.gray(' (on-chain)')); }
    catch { /* */ }
  }
  if (active === 'false') log.kv('sale state', chalk.yellow.bold('NOT OPEN') + chalk.gray(' (saleIsActive=false)'));
  else if (active === 'true') log.kv('sale state', chalk.green.bold('OPEN'));
  if (paused === 'true') log.kv('paused', chalk.red.bold('TRUE'));
  if (startTs && Number(startTs) > 0) {
    const ts = Number(startTs);
    const now = Math.floor(Date.now() / 1000);
    if (ts > now) log.kv('starts in', chalk.yellow(`${Math.floor((ts - now) / 60)}m${(ts - now) % 60}s`) + chalk.gray(`  (${new Date(ts * 1000).toISOString()})`));
    else log.kv('started at', chalk.gray(new Date(ts * 1000).toISOString()));
  }
  ...
  console.log();
  if (active === 'false') log.warn('sale flag is FALSE — wait for it to flip before minting');
  else if (paused === 'true') log.warn('contract paused — wait for unpause');
...
async function runMintFlow(cfg: Config, flags: Flags, mode: 'test' | 'mint', autoYes: boolean) {
  const provider = new JsonRpcProvider(cfg.rpcUrl, cfg.chainId, { staticNetwork: true });

  // Probe contract FIRST so we can auto-fill price / start time / etc.
  log.step('Probing contract on-chain');
  let initialProbes: ProbeResult[] = [];
  ...
  if (cfg.adaptive) log.kv('priority', `auto p${cfg.adaptive.percentile} + ${cfg.adaptive.premiumGwei} gwei`);
  else              log.kv('priority', cfg.maxPriorityFeeGwei + ' gwei');
  if (cfg.mintStartTs) {
    log.kv('mint opens', new Date(cfg.mintStartTs * 1000).toISOString() + (cfg.startExplicit ? '' : chalk.green('  (auto)')));
  }
  if (cfg.mintStartBlock) log.kv('mint opens', `block ${cfg.mintStartBlock}`);
  if (flags.to) log.kv('→ transfer to', chalk.cyan(flags.to) + chalk.gray('  (after mint)'));
  ...
  if (cfg.mintStartTs || cfg.mintStartBlock) {
    ...
    try {
      ...
      const getNow = (k: string) => now.find((x) => x.label === k)?.value;
      const getOld = (k: string) => initialProbes.find((x) => x.label === k)?.value;
      ...
      if (active === 'false') log.warn('sale flag still FALSE — your tx may revert');
      if (active === 'true')  log.ok('sale flag is TRUE — open');
      const paused = getNow('paused');
      if (paused === 'true') log.err('contract is PAUSED — will revert');
    } catch (e: any) {
    ...
    if (cfg.mintStartTs)    await waitUntilUnix(cfg.mintStartTs);
    if (cfg.mintStartBlock) await waitUntilBlock(provider, cfg.mintStartBlock);
  }
  ...
  try {
    ...
    if (txRes?.error) {
      ...
      if (mode === 'test') process.exit(2);
      if (!autoYes && !(await confirm('Send anyway?'))) process.exit(1);
    } else {
  ...
  const jobs = [] as Array<Promise<{ relay: string; target: number; bundleHash: string } | null>>;
  for (let i = 1; i <= cfg.blocksToTarget; i++) {
    const target = prepared.blockNumber + i;
    for (const r of relays) {
      const host = new URL(r.url).hostname;
      jobs.push(
        r.fb.sendBundle([prepared.signedTx], target)
          .then((res) => {
            submittedAny = true;
...
async function main() {
  ...
  if (flags.command === 'help' || flags.command === 'unknown') { printHelp(); return; }
  const cfg = loadConfig(flags);
  if (flags.command === 'check') return checkMode(cfg);
