import {
  JsonRpcProvider, Wallet, Interface, Transaction, getAddress, parseUnits,
} from 'ethers';
...
function resolveArg(a: unknown, minter: string, qty: number): unknown {
  if (a === 'self') return minter;
  if (a === 'qty' || a === 'quantity') return qty;
  return a;
}
